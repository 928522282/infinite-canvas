...
+ [新增] 画布内置逐 VID 视频工作流导出，按 VID-001、VID-002 等独立目录打包本幕人物与场景资产、关键参考图及生图提示词、优化视频提示词和机器清单。
+ [新增] 提示词中心加入“关键帧生图提示词｜资产强约束版”Skill，以已确认人物和场景资产锁定脸部骨骼、妆造、空间坐标、透视与光源，并保留旧版供对比。
+ [新增] 提示词中心加入画布适配的“人物微表情表演增强”Skill，在确认参考图与视频镜头编译之间按 VID 输出分时表演层。
+ [调整] 短剧画布生图改为以上一张一致性校验通过的确认图作为下一帧身份参考，并在跨帧人物漂移时阻断分支。

+ [调整] 短剧工作流将完整 Skill 快照内嵌到画布节点，执行时不再依赖本机 Skill 文件，提示词中心修改会更新已有画布快照，生图步骤统一使用画布原生功能。
+ [调整] 短剧工作流在脚本转化后按 VID 独立执行每个后续 Skill，以直接连线和持久化来源元数据保留逐节点溯源，单条分支阻断不再影响其他 VID。
+ [调整] AI 脚本转化结果按 SCENE 创建画布分组、按 VID 创建完整独立文本节点，转化主节点只保留进度与索引供工作流查看。
+ [修复] Canvas Agent 创建节点协议补充真实 Group 类型，使 Codex 能自动组织分组工作流。
+ [调整] 短剧工作流执行文件输入时先在画布展示完整原文、提取进度与可追溯来源，原文未完整落入输入节点前不再启动首个 Skill。
+ [新增] 提示词中心加入“脚本人物资产关联审核”Skill，并将其置于默认工作流的脚本转化与人物生图提示词之间。
+ [修复] 输入提示词工作流时兼容已安装 Skill 的旧版短说明，不再因不足 25 个字符而中断同步。
+ [调整] 提示词中心成为内置短剧 Skill 的唯一编辑入口，首次同步会完整复制同名 Skill 的脚本、参考资料与资产，后续保存和恢复原稿通过 revision 安全更新工作区 `SKILL.md`，工作流输入前再次校准正文。
+ [新增] 提示词中心加入“首帧与重要镜头参考图提示词”和“优化视频镜头提示词编译器”，并扩展短剧工作流以生成参考图和 MiniMax H3 视频Prompt。
+ [新增] 提示词中心支持加入、移出和前后调整 Skill 卡片，输入画布时按用户编排创建线性节点，并为 Codex 预填严格顺序执行指令。
+ [优化] 为五张内置短剧工作流提示词卡片补齐 SilkDock 写实风封面，并按 3:4 画幅完整展示。
+ [新增] 提示词卡片支持浏览器本地编辑和一键恢复原稿，修改结果同步用于搜索、选择和画布输入。
+ [新增] 提示词中心支持选择目标画布，并将完整Prompt作为可编辑文本节点输入画布。
+ [新增] 提示词中心内置AI脚本转化和关键帧生图提示词两张短剧工作流卡片。
+ [修复] 画布接入 SilkDock 异步生图任务轮询，并兼容其参考图上传字段。
+ [调整] 提示词中心移除全部默认及已保存的远程来源，并在升级时清理旧提示词缓存，为后续本地提示词管理做准备。
+ [新增] Web 端接入 Codex Sites 私有托管构建，可从同一画布源码持续发布更新。
